'''
############################################################################################################################
Risk Assessment for Solar PV Scenario using Bayesian Belief Networks (BBN) based on AutomationML Models
...
def generate_cpd_values_occurrence(num_states, num_parents, hazard_node=False, vulnerability_node=False, process_node=False):
    ...
    if hazard_node:
        ...
        if ref_base_for_node in ['AssetOfICS/Hardware/Process device/Actuator',
                                 ...
                                 'AssetOfICS/Hardware/Process device/Controller',
                                 'AssetOfICS/Hardware/Process device/Workstation',
                                 'AssetOfICS/Software/Application']:
...
if __name__ == "__main__":
    ...
    for node in bbn_occurrence.nodes():
        ...
        #print(f"CPD values before normalization for node {node}: {cpd_values}")
...
while True:
    source_node_input = input("Enter source node (Attacker, V1-V16) or leave blank for default (Attacker)): ").strip()
    if not source_node_input:  # Default to "Attacker" if input is blank
