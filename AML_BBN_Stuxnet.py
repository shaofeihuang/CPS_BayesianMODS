...
'''
print("\nPaths from each source node to 'CPS_Compromise':")
...
for nodes in total_elements:
    if nodes==last_node:
        ...
        print("[*] CPT (Severity/Impact) of Centrifuge Failure:\n", impact_termination)        
        cpd_prob = prob_termination.values
        cpd_impact = impact_termination.values
        print('--------------------------------------------------------')
        print("[*] Posterior probability of Centrifuge Failure:", cpd_prob[0])
        print("[*] Posterior severity/impact of Centrifuge Failure:", cpd_impact[0])
