'''
############################################################################################################################
Optuna Multi-Objective Optimization / BlackEnergy Scenario using Bayesian Belief Networks (BBN) based on AutomationML Models
...
############################################################################
def generate_cpd_values_exposure(num_states, num_parents, node, matching_nodes, hazard_node=False, vulnerability_node=False, process_node=False):
    ...
    if hazard_node:
        ...
        if matching_nodes[0]['ID'] in [f"V{i}" for i in range(1,17)]:  # Check if ID is V1 to V16
...
#################################################################################################################
            ...
            if matching_nodes[0]['ID'] in ["Solar_PV_Inverter"]:
            ...
            if matching_nodes[0]['ID'] in ["Comms_Dongle"]:
            ...
            if matching_nodes[0]['ID'] == "Power_Grid":
                values = [
                    float(element['Probability of Mitigation'])
                    for element in VulnerabilityinSystem
                    if element['ID'] == "V15"
                ...
                if values:
                    V15 = values[0]
                    if V15 > 0:
                        probability_of_failure_for_node = 1 - V15
...
def generate_cpd_values_impact_(num_states, num_parents, node, matching_nodes, hazard_node=False, vulnerability_node=False, process_node=False):
    cpd_values = np.zeros((num_states, 2 ** num_parents))
    current_entry = next((entry for entry in result_list if entry['Element'] == node), None)
...
def bbn_inference():
    ...
    for node in bbn_exposure.nodes():
        ...
        matching_process_nodes = [element for element in AssetinSystem if element['ID'] == node]
        ...
        if matching_hazard_nodes:
            ...
            cpd_values = generate_cpd_values_exposure(num_states, num_parents, node, matching_process_nodes, process_node=True)
    ...
    for node in bbn_impact.nodes():
        ...
        if matching_hazard_nodes:
            ...
            cpd_values = generate_cpd_values_impact_(num_states, num_parents, node, matching_process_nodes, process_node=True)
    ...
    for nodes in total_elements:
        if nodes == last_node:
            ...
            impact_score = compute_impact(bbn_exposure, source_node, last_node)
...
def objective(trial):
    ...
    for element in VulnerabilityinSystem:
        if element['ID'] in [f"V{i}" for i in range(1,17)]:  # Check if ID is V1 to V16
